...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "paperclip_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Nova Corps skills are read-only.",
      sourceLabel: "Nova Corps bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Nova Corps workspace",
        sourceBadge: "paperclip",
    ...
    for (const skill of skills) {
      ...
      out.push({
        ...
        requiredReason: required
          ? "Bundled Nova Corps skills are always available for local adapters."
          : null,
      });
    }
