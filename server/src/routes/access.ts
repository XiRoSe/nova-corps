...
/** Resolve the Nova Corps repo skills directory (built-in / managed skills). */
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Nova Corps host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Nova Corps is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm nova-corps allowed-hostname ${apiHost}`
    });
  }
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Nova Corps hostname is reachable and allowed via `pnpm nova-corps allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Nova Corps API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Nova Corps OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Nova Corps generates and persists one during join so pairing approvals are stable.
    ...
    The board approves the join request in Nova Corps before key claim is allowed.
    ...
    ## Step 4: Install Nova Corps skill in OpenClaw
    ...
    ${
      ...
      "Ensure Nova Corps is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Nova Corps base URLs to try");
    ...
    appendBlock(`
      ...
      - pnpm nova-corps allowed-hostname <host>
      - then restart Nova Corps and retry onboarding.
    `);
  }
